...
delete_local_backups="$(bashio::config 'delete_local_backups' 'false')"
...
bashio::log.debug "Command: 'aws s3 sync $monitor_path s3://$bucket_name/ --no-progress --region $bucket_region --storage-class $storage_class  --endpoint-url $endpoint_url'"
aws s3 sync $monitor_path s3://"$bucket_name"/ --no-progress --region "$bucket_region" --storage-class "$storage_class" --endpoint-url "$endpoint_url"
